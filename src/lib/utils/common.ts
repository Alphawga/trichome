...
export function generateSKU(productName: string, variant?: string): string {
  const cleanName = productName.replace(/[^a-zA-Z0-9]/g, '').substring(0, 6).toUpperCase()
  const variantCode = variant ? variant.replace(/[^a-zA-Z0-9]/g, '').substring(0, 3).toUpperCase() : ''
