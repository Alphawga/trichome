...
export function PromotionViewSheet({ promotion, open, onOpenChange }: PromotionViewSheetProps) {
  if (!promotion) return null;

  const usageCount = promotion._count?.usages || 0;
  const usageLimit = promotion.usage_limit;
  const usagePercentage = usageLimit > 0 ? Math.min((usageCount / usageLimit) * 100, 100) : 0;
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gradient-to-r from-green-50 to-blue-50 p-6 rounded-lg border border-green-200">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-2xl font-bold text-gray-900">{promotion.name}</h3>
                <code className="inline-block mt-2 bg-white px-3 py-1.5 rounded text-lg font-mono border border-gray-300">
                  {promotion.code}
                </code>
              </div>
              <span className={`px-3 py-1.5 text-sm font-semibold rounded-full ${statusColors[promotion.status]}`}>
                {promotion.status}
              </span>
            </div>
            ...
            <div className="flex items-center gap-6 mt-4">
              ...
              {promotion.max_discount && (
                <div className="text-center border-l pl-6">
                  ...
                  <p className="text-xl font-semibold">₦{Number(promotion.max_discount).toLocaleString()}</p>
                </div>
              )}
            </div>

            {promotion.description && (
              <p className="mt-4 text-gray-700 italic">{promotion.description}</p>
            )}
          </div>
              ...
              {promotion.usage_limit_per_user && (
                <div className="pt-3 border-t">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-600">Per Customer Limit</span>
                    <span className="font-semibold">{promotion.usage_limit_per_user} uses</span>
                  </div>
                </div>
              )}
              ...
              <div className="pt-3 border-t">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-green-600">
                    {usageLimit > 0 ? Math.max(0, usageLimit - usageCount) : '∞'}
                  </span>
                </div>
              </div>
            </div>
          ...
          {isScheduled && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-800 font-medium">
                ℹ️ This promotion is scheduled and will start on {new Date(promotion.start_date).toLocaleDateString()}
              </p>
            </div>
          )}
