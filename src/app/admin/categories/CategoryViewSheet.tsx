...
export function CategoryViewSheet({ categoryId, open, onOpenChange }: CategoryViewSheetProps) {
        ...
        {isLoading ? (
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              ...
              <p className="text-gray-600 mb-3">{category.description || 'No description'}</p>
              <div className="flex items-center gap-3">
                <span className={`px-3 py-1 text-sm font-semibold rounded-full ${
                  category.status === ProductStatus.ACTIVE ? 'bg-green-100 text-green-800' :
                  category.status === ProductStatus.DRAFT ? 'bg-yellow-100 text-yellow-800' :
                  category.status === ProductStatus.INACTIVE ? 'bg-gray-100 text-gray-800' :
            ...
            <div className="border-t pt-6">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-base text-gray-900 font-mono bg-gray-50 px-3 py-2 rounded">{category.slug}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">Sort Order</p>
                  <p className="text-base text-gray-900">{category.sort_order}</p>
                </div>
                {category.parent && (
                  <div>
                    <p className="text-sm text-gray-600 mb-1">Parent Category</p>
                    <p className="text-base text-gray-900">{category.parent.name}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="border-t pt-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-blue-900">{category._count.products}</p>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg">
                  <p className="text-sm text-purple-600 mb-1">Subcategories</p>
                  <p className="text-2xl font-bold text-purple-900">{category.children.length}</p>
                </div>
              </div>
            </div>
                  ...
                  {category.children.map((child) => (
                    <div key={child.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      ...
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                        child.status === ProductStatus.ACTIVE ? 'bg-green-100 text-green-800' :
                        child.status === ProductStatus.DRAFT ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="border-t pt-6">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{new Date(category.created_at).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Last Updated</span>
                  <span className="text-gray-900">{new Date(category.updated_at).toLocaleString()}</span>
                </div>
              </div>
            </div>
          </div>
