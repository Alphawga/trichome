...
type Customer = Pick<
  User,
  'id' | 'email' | 'first_name' | 'last_name' | 'phone' | 'status' | 'image' | 'created_at' | 'last_login_at'
> & {
  totalOrders: number;
  totalSpent: number;
  lastOrderDate?: Date;
...
export function CustomerViewSheet({ customer, open, onOpenChange }: CustomerViewSheetProps) {
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <span className={`inline-block px-2 py-1 text-xs font-semibold rounded-full ${statusStyles[customer.status]}`}>
              ...
              <div>
                ...
                <p className="font-medium text-[#38761d]">{customer.loyaltyPoints.toLocaleString()} pts</p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl font-bold text-orange-600">
                    {customer.totalSpent >= 500000 ? 'Platinum' :
                     customer.totalSpent >= 200000 ? 'Gold' :
                     customer.totalSpent >= 50000 ? 'Silver' : 'Bronze'}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-gray-600">Lifetime Value</p>
                  <p className="text-xl font-bold text-[#38761d]">₦{customer.totalSpent.toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
