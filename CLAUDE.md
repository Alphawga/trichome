...
`src/lib/shipping/calculate-shipping.ts` computes cost from a per-state NGN rate table (`STATE_SHIPPING_COSTS`) plus a free-shipping threshold (₦50,000); `src/lib/shipping/tracking-service.ts` handles order tracking status.
